...
public class HighestCarouselRanking {
	...
	public static void finalCarousels(HashMap<String, Integer> downwardRId, HashMap<String, Integer> sidewardRId) throws IOException {
		//Reader dcreader; Reader screader;
		
		//===============Data For Downward Carousel=================
		FileInputStream fileToWrite = new FileInputStream(new File(".//FinalDownwardCarousel.xlsx"));
		XSSFWorkbook workbookToWrite = new XSSFWorkbook(fileToWrite); 
		XSSFSheet sheetToWrite = workbookToWrite.getSheetAt(0);
        FileOutputStream outFile = new FileOutputStream(new File(".//FinalDownwardCarousel.xlsx"));
        
      //===============Data For Sideward Carousel=================
        FileInputStream fileToWrite1 = new FileInputStream(new File(".//FinalSidewardCarousel.xlsx"));
		XSSFWorkbook workbookToWrite1 = new XSSFWorkbook(fileToWrite1); 
		XSSFSheet sheetToWrite1 = workbookToWrite1.getSheetAt(0);
        FileOutputStream outFile1 = new FileOutputStream(new File(".//FinalSidewardCarousel.xlsx"));
		...
		while (rowIterator1.hasNext()) {
			...
			String title = null; String facts = null; String pivotEntity = null;
			...
			for(int i : downwardRId.values()) {
				if(row.getRowNum() == i) {
					...
					Cell factCell = rowToWrite.createCell(2);
					factCell.setCellValue(facts);
		...
		File file1 = new File(".//FinalDownwardCarousel.csv");
        File file2 = new File(".//FinalSidewardCarousel.csv");
        FileWriter outputFile1 = new FileWriter(file1); FileWriter outputFile2 = new FileWriter(file2);
		...
		for(CSVRecord dccsvrecord : dccsvParser) {
			for(int i : downwardRId.values()) {
				if(dccsvrecord.getRecordNumber() == i) {
					String data[] = {dccsvrecord.get(0), dccsvrecord.get(1), dccsvrecord.get(2), dccsvrecord.get(3)};
		...
		for(CSVRecord sccsvrecord : sccsvParser) {
			for(int i : sidewardRId.values()) {
				if(sccsvrecord.getRecordNumber() == i) {
					String data[] = {sccsvrecord.get(0), sccsvrecord.get(1), sccsvrecord.get(2), sccsvrecord.get(3)};
	...
	public static void main(String[] args) throws IOException {
		...
		FileInputStream dcfile = new FileInputStream(new File(".//DownwardCarouselRanking.xlsx"));	        
        FileInputStream scfile = new FileInputStream(new File(".//SidewardCarouselRanking.xlsx"));
		...
		HashMap<String, Double> peDownward = new HashMap<String, Double>(); HashMap<String, Integer> downwardRId = new HashMap<String, Integer>();
		HashMap<String, Double> peSideward = new HashMap<String, Double>(); HashMap<String, Integer> sidewardRId = new HashMap<String, Integer>();
		
		while (rowIterator1.hasNext()) {
			Row row = rowIterator1.next();
			double score = 0; String pivotEntity = null;
		...
		System.out.println("===============================");
		/*dcreader = Files.newBufferedReader(Paths.get(".//DownwardCarouselRanking.csv"),Charset.forName("ISO-8859-1"));
		screader = Files.newBufferedReader(Paths.get(".//SidewardCarouselRanking.csv"),Charset.forName("ISO-8859-1"));
		CSVParser dccsvParser = new CSVParser(dcreader, CSVFormat.DEFAULT);
		CSVParser sccsvParser = new CSVParser(screader, CSVFormat.DEFAULT);
		HashMap<String, Double> peDownward = new HashMap<String, Double>(); HashMap<String, Integer> downwardRId = new HashMap<String, Integer>();
		HashMap<String, Double> peSideward = new HashMap<String, Double>(); HashMap<String, Integer> sidewardRId = new HashMap<String, Integer>();
		...
		for(CSVRecord dccsvrecord : dccsvParser) {
			//System.out.println(dccsvrecord.get(1));
			if(peDownward.keySet().isEmpty()) {
			...
			for(Map.Entry<String, Double> peEntry : peDownward.entrySet()) {
				if(peEntry.getKey().equals(dccsvrecord.get(1))) {
					if(peEntry.getValue() < Double.parseDouble(dccsvrecord.get(2))) {
						peDownward.put(peEntry.getKey(), Double.parseDouble(dccsvrecord.get(2)));
						downwardRId.put(dccsvrecord.get(1), (int) dccsvrecord.getRecordNumber()+1);
					}
				}
		...
		for(CSVRecord sccsvrecord : sccsvParser) {
			...
			for(Map.Entry<String, Double> peEntry : peSideward.entrySet()) {
				if(peEntry.getKey().equals(sccsvrecord.get(1))) {
					if(peEntry.getValue() < Double.parseDouble(sccsvrecord.get(2))) {
						peSideward.put(peEntry.getKey(), Double.parseDouble(sccsvrecord.get(2)));
						sidewardRId.put(sccsvrecord.get(1), (int) sccsvrecord.getRecordNumber()+1);
					}
				}
		...
		System.out.println(peDownward); System.out.println(downwardRId);
		System.out.println("==========================================");
		System.out.println(peSideward); System.out.println(sidewardRId);
		//finalCarousels(downwardRId, sidewardRId);
	}
